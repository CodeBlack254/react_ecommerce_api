...
const CartItem = ({ item }) => {
  ...
  return <div className='flex gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500'>
    <div className='w-full min-h-[150px] flex items-center gap-x-4'>
      <Link to={`/product/${id}`}>
        <img className='max-w-[80px]' src={image} alt='prduct image' />
      </Link>
      <div className='w-full flex flex-col'>
        ...
        <div className='flex gap-x-2 h-[36px] text-sm'>
          ...
          <div className='flex-1 flex justify-end items-centertext-primary font-medium'>{`$ ${parseFloat(price * amount).toFixed(2)}`} </div>
        </div>
      </div>
